...
export function createRingAdapter(flatView: Float32Array, chunkSamples: number): Ring {
  const length = flatView.length / chunkSamples
  return Array.from({ length }, (_, i) => flatView.subarray(i * chunkSamples, (i + 1) * chunkSamples)) as Ring
}
